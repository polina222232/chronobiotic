...
class Synonyms(models.Model):
    ...
    originalbiotic = models.ForeignKey(
        "Chronobiotic", on_delete=models.CASCADE, blank=True, related_name="synonyms"
    )
...
class Articles(models.Model):
    ...
    articleurl = models.URLField(
        max_length=250, help_text="Enter a link about this target"
    )
...
class Targets(models.Model):
    ...
    targetsfullname = models.CharField(
        max_length=150, blank=True, help_text="Enter a fullname"
    )
    targeturl = models.URLField(
        max_length=200, help_text="Enter a link about this target"
    )
...
class Mechanism(models.Model):
    ...
    mechanismname = models.CharField(
        max_length=128, help_text="Enter a mechanism(e.g. relax, energy, tea , coffe)"
    )
...
class Bioclass(models.Model):
    ...
    nameclass = models.CharField(
        max_length=128, help_text="Enter a class(e.g. chronobiotics, radioprotectors)"
    )
...
class Chronobiotic(models.Model):
    ...
    description = models.TextField(
        max_length=5000, help_text="Enter a  description of the biotic", blank=True
    )
    ...
    classf = models.ManyToManyField(
        Bioclass, help_text="Select a class for this molecula"
    )
    mechanisms = models.ManyToManyField(
        Mechanism, help_text="Select a mechanism of this "
    )
    target = models.ManyToManyField(Targets, help_text="Select a targets of this ")
    effect = models.ManyToManyField(
        Effect, help_text="Select a effects of this ", blank=True
    )
